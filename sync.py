...
Fetches a Spotify playlist, downloads each track via spotdl, extracts
...
redirectUri resolution:
  - If GITHUB_PAGES_URL env var is set → use that URL (set by the action)
  - Otherwise → http://localhost:8080 (local dev)
...
def resolve_redirect_uri(config: dict) -> str:
    """Return the OAuth redirect URI appropriate for the current environment."""
...
def write_config_js(webapp_dir: Path, client_id: str, redirect_uri: str) -> None:
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    redirect_uri = resolve_redirect_uri(config)
    print(f"\nWriting config.js (redirectUri={redirect_uri})...")
    write_config_js(webapp_dir, config["client_id"], redirect_uri)
